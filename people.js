class humain {
  ...
  card(index) {
    let background =
      this.gender == "Femme" ? "card-body-female" : "card-body-male";

    return `
    <div id="card2" class="${background} card col-lg-3 col-12 px-0 m-3 text-center shadow border border-3">
        <img src="img/${this.picture}" class="card-img-top" alt="${this.id}">
        <div class="">
            <h5 class="card-title">${this.pseudo}, ${this.age}</h5>
            <button onclick="goProfile(${index})" id="btn1" class="btn btn-light"><b>Découvrir</b></button>
        </div>
    </div>
    `;
  }
